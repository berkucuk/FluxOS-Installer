...
class ServerProfile(Profile):
	...
	def packages(self) -> list[str]:
		return [
			...
			'efibootmgr',
			'mkinitcpio',
			# Server-specific packages from releng/packages.x86_64
			...
			'bind',
			'dnsmasq',
			# Web servers (optional - can be installed separately)
			# 'nginx',
			# 'apache',
			# Database servers (optional - can be installed separately)
			# 'mariadb',
			# 'postgresql',
			# 'redis',
			# Container & orchestration (optional - can be installed separately)
			# 'docker',
			# 'kubernetes',
			# Configuration management (optional - can be installed separately)
			# 'ansible',
			# 'terraform',
		]
