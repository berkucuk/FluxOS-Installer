...
class Installer:
	...
	def minimal_installation(
		...
		# Install FluxOS configuration files
		self._install_fluxos_configs()
	...
	def _add_grub_bootloader(
		...
		# Customize GRUB for FluxOS
		config = re.sub(r'(GRUB_DISTRIBUTOR=).*', r'\1"FluxOS"', config)
		if 'GRUB_DISTRIBUTOR=' not in config:
			config += '\nGRUB_DISTRIBUTOR="FluxOS"\n'
	...
	def _install_fluxos_configs(self) -> None:
		"""Install FluxOS specific configuration files"""
		info('Installing FluxOS configuration files...')
		
		try:
			# Get the archinstall package directory
			import archinstall
			archinstall_files = files(archinstall)
			
			# Check if fluxos_config_files exists
			try:
				config_source = archinstall_files / 'fluxos_config_files'
				# Test if directory exists by trying to iterate
				list(config_source.iterdir())
			except (FileNotFoundError, AttributeError):
				warn('FluxOS configuration files not found in package')
				return
			
			# Install SDDM themes
			sddm_themes_target = self.target / 'usr/share/sddm/themes'
			sddm_themes_target.mkdir(parents=True, exist_ok=True)
			
			# Copy FluxOS SDDM themes
			try:
				fluxos_breeze_source = config_source / 'sddm_theme/fluxos-breeze'
				fluxos_breeze_target = sddm_themes_target / 'fluxos-breeze'
				fluxos_breeze_target.mkdir(parents=True, exist_ok=True)
				
				# Copy each file individually since we're dealing with resources
				for item in fluxos_breeze_source.iterdir():
					if item.is_file():
						target_file = fluxos_breeze_target / item.name
						with item.open('rb') as src, open(target_file, 'wb') as dst:
							dst.write(src.read())
					elif item.name != '__pycache__':
						# For directories, copy recursively
						item_target = fluxos_breeze_target / item.name
						item_target.mkdir(parents=True, exist_ok=True)
						try:
							for subitem in item.iterdir():
								if subitem.is_file():
									with subitem.open('rb') as src, open(item_target / subitem.name, 'wb') as dst:
										dst.write(src.read())
						except:
							pass  # Skip if directory iteration fails
				
				info('Installed FluxOS Breeze SDDM theme')
			except:
				warn('Failed to install FluxOS Breeze SDDM theme')
			
			# Install FluxAI Chat application with virtual environment
			try:
				fluxai_source = config_source / 'sddm_theme/fluxai-chat'
				fluxai_target = self.target / 'usr/share/flux-ai-chat'
				fluxai_target.mkdir(parents=True, exist_ok=True)
				
				# Copy all FluxAI files
				for item in fluxai_source.iterdir():
					if item.is_file():
						target_file = fluxai_target / item.name
						with item.open('rb') as src, open(target_file, 'wb') as dst:
							dst.write(src.read())
						# Make executable if it's a script
						if item.name.endswith(('.py', '.sh')):
							target_file.chmod(0o755)
					elif item.name != '__pycache__':
						# For directories like icons/, copy recursively
						item_target = fluxai_target / item.name
						item_target.mkdir(parents=True, exist_ok=True)
						try:
							for subitem in item.iterdir():
								if subitem.is_file():
				...
				# Install essential Python packages via pacman for base system
				essential_deps = ['python', 'python-pip', 'python-virtualenv']
				try:
					self.pacman.strap(essential_deps)
				except:
					warn('Failed to install essential Python packages')
				
				# Create virtual environment for FluxAI
				venv_path = fluxai_target / 'python-env'
				venv_abs_path = f'/usr/share/flux-ai-chat/python-env'  # Absolute path for chroot
				try:
					# Create virtual environment
					SysCommand(f'arch-chroot {self.target} python3 -m venv {venv_abs_path}')
					
					# Upgrade pip in virtual environment
					SysCommand(f'arch-chroot {self.target} {venv_abs_path}/bin/pip install --upgrade pip')
					
					# Install dependencies from requirements.txt if exists
					requirements_file = fluxai_target / 'requirements.txt'
					if requirements_file.exists():
						SysCommand(f'arch-chroot {self.target} {venv_abs_path}/bin/pip install -r /usr/share/flux-ai-chat/requirements.txt')
						info('Installed FluxAI dependencies in virtual environment')
					else:
						# Fallback: install essential dependencies manually
						essential_packages = [
							'requests', 'pillow', 'psutil', 'PyQt5', 
							'python-dateutil', 'pytz', 'python-dotenv',
							'langchain-core', 'langchain-google-genai', 
							'google-generativeai', 'gtts', 'pygame', 'distro'
						]
						for package in essential_packages:
							try:
								SysCommand(f'arch-chroot {self.target} {venv_abs_path}/bin/pip install {package}')
							except:
								pass  # Continue if package installation fails
						
						info('Installed FluxAI fallback dependencies in virtual environment')
					
					# Make sure virtual environment python is executable
					if not (self.target / f'{venv_abs_path}/bin/python3').exists():
						# Create symlink if python3 doesn't exist
						SysCommand(f'arch-chroot {self.target} ln -sf {venv_abs_path}/bin/python {venv_abs_path}/bin/python3')
					
					# Create a wrapper script for easier execution
					wrapper_script = fluxai_target / 'flux-ai-launcher.sh'
					with open(wrapper_script, 'w') as f:
						f.write('#!/bin/bash\n')
						f.write(f'cd /usr/share/flux-ai-chat\n')
						f.write(f'{venv_abs_path}/bin/python3 /usr/share/flux-ai-chat/flux_ai.py "$@"\n')
					wrapper_script.chmod(0o755)
					
				except Exception as e:
					warn(f'Failed to create virtual environment for FluxAI: {e}')
					# Fallback: try to install system-wide
					try:
						SysCommand(f'arch-chroot {self.target} pip install --break-system-packages -r {fluxai_target}/requirements.txt')
					except:
						warn('Failed to install FluxAI dependencies system-wide as well')
				
				# Install desktop file to system applications
				desktop_target = self.target / 'usr/share/applications'
				desktop_target.mkdir(parents=True, exist_ok=True)
				desktop_file = fluxai_target / 'Flux-AI.desktop'
				if desktop_file.exists():
					shutil.copy2(desktop_file, desktop_target)
				
				info('Installed FluxAI Chat application with virtual environment')
			except:
				warn('Failed to install FluxAI Chat application')
			
			# Install wallpapers and images
			try:
				images_source = config_source / 'sddm_theme/images'
				wallpapers_target = self.target / 'usr/share/pixmaps/fluxos'
				wallpapers_target.mkdir(parents=True, exist_ok=True)
				
				for image in images_source.iterdir():
					if image.is_file() and image.name.lower().endswith(('.png', '.jpg', '.jpeg')):
						target_file = wallpapers_target / image.name
						with image.open('rb') as src, open(target_file, 'wb') as dst:
							dst.write(src.read())
				
				info('Installed FluxOS wallpapers and images')
			except:
				warn('Failed to install FluxOS wallpapers and images')
			
			# Configure SDDM to use FluxOS theme
			sddm_conf_dir = self.target / 'etc/sddm.conf.d'
			sddm_conf_dir.mkdir(parents=True, exist_ok=True)
			
			fluxos_sddm_conf = sddm_conf_dir / 'fluxos.conf'
			with open(fluxos_sddm_conf, 'w') as f:
				f.write('[Theme]\n')
				f.write('Current=fluxos-breeze\n')
				f.write('\n[General]\n')
				f.write('Numlock=on\n')
			
			info('Configured SDDM to use FluxOS theme')
			
			# Set FluxOS wallpaper as system default
			try:
				fluxos_wallpaper_script = self.target / 'usr/local/bin/set-fluxos-wallpaper.sh'
				fluxos_wallpaper_script.parent.mkdir(parents=True, exist_ok=True)
				
				with open(fluxos_wallpaper_script, 'w') as f:
					f.write('#!/bin/bash\n')
					f.write('# FluxOS Default Wallpaper Script\n')
					f.write('# This script sets FluxOS wallpaper as default for all users\n\n')
					f.write('WALLPAPER_PATH="/usr/share/pixmaps/fluxos/fluxos-wallpaper.png"\n\n')
					f.write('# Check if wallpaper exists\n')
					f.write('if [ ! -f "$WALLPAPER_PATH" ]; then\n')
					f.write('    echo "FluxOS wallpaper not found at $WALLPAPER_PATH"\n')
					f.write('    exit 1\n')
					f.write('fi\n\n')
					f.write('# Set wallpaper for current user if running in X11/Wayland\n')
					f.write('if [ -n "$DISPLAY" ] || [ -n "$WAYLAND_DISPLAY" ]; then\n')
					f.write('    if command -v plasma-apply-wallpaperimage >/dev/null 2>&1; then\n')
					f.write('        plasma-apply-wallpaperimage "$WALLPAPER_PATH"\n')
					f.write('        echo "Applied FluxOS wallpaper using plasma-apply-wallpaperimage"\n')
					f.write('    elif command -v qdbus >/dev/null 2>&1; then\n')
					f.write('        # Alternative method using qdbus\n')
					f.write('        qdbus org.kde.plasmashell /PlasmaShell org.kde.PlasmaShell.evaluateScript "\n')
					f.write('            var allDesktops = desktops();\n')
					f.write('            for (i=0;i<allDesktops.length;i++) {\n')
					f.write('                d = allDesktops[i];\n')
					f.write('                d.wallpaperPlugin = \\"org.kde.image\\";\n')
					f.write('                d.currentConfigGroup = Array(\\"Wallpaper\\", \\"org.kde.image\\", \\"General\\");\n')
					f.write('                d.writeConfig(\\"Image\\", \\"file://$WALLPAPER_PATH\\");\n')
					f.write('            }"\n')
					f.write('        echo "Applied FluxOS wallpaper using qdbus"\n')
					f.write('    fi\n')
					f.write('fi\n')
				
				fluxos_wallpaper_script.chmod(0o755)
				
				# Create autostart entry for wallpaper script
				autostart_dir = self.target / 'etc/xdg/autostart'
				autostart_dir.mkdir(parents=True, exist_ok=True)
				
				autostart_file = autostart_dir / 'fluxos-wallpaper.desktop'
				with open(autostart_file, 'w') as f:
					f.write('[Desktop Entry]\n')
					f.write('Type=Application\n')
					f.write('Name=FluxOS Wallpaper\n')
					f.write('Comment=Set FluxOS default wallpaper\n')
					f.write('Exec=/usr/local/bin/set-fluxos-wallpaper.sh\n')
					f.write('Terminal=false\n')
					f.write('Hidden=false\n')
					f.write('StartupNotify=false\n')
					f.write('OnlyShowIn=KDE;\n')
					f.write('X-KDE-autostart-condition=kdesktoprc:General:AutostartEnabled:true\n')
					f.write('Categories=System;\n')
				
				info('Created FluxOS wallpaper auto-apply script')
				
			except Exception as e:
				warn(f'Failed to create wallpaper script: {e}')
			
			# Install KDE configurations and additional FluxOS assets
			try:
				kde_configs_source = config_source / 'sddm_theme/kde_configs'
				if kde_configs_source.exists():
					# Create skeleton home directory for default user configs
					skel_dir = self.target / 'etc/skel'
					...
					def copy_recursive(src_dir, dst_dir):
						for src_item in src_dir.iterdir():
							...
							if src_item.is_file():
								with src_item.open('rb') as src_f, open(dst_item, 'wb') as dst_f:
									dst_f.write(src_f.read())
							elif src_item.is_dir() and src_item.name != '__pycache__':
								dst_item.mkdir(parents=True, exist_ok=True)
								copy_recursive(src_item, dst_item)
					
					# Copy .local directory if exists
					local_source = kde_configs_source / '.local'
					if local_source.exists():
						skel_local = skel_dir / '.local'
						skel_local.mkdir(parents=True, exist_ok=True)
						copy_recursive(local_source, skel_local)
						info('Copied .local configurations to skeleton directory')
					
					# Copy .config directory if exists  
					config_source_dir = kde_configs_source / '.config'
					if config_source_dir.exists():
						skel_config = skel_dir / '.config'
						skel_config.mkdir(parents=True, exist_ok=True)
						copy_recursive(config_source_dir, skel_config)
						info('Copied .config configurations to skeleton directory')
					
					info('Installed KDE configurations to skeleton directory')
				
				# Install additional wallpapers to system wallpaper directories
				kde_wallpapers_target = self.target / 'usr/share/wallpapers/fluxos'
				kde_wallpapers_target.mkdir(parents=True, exist_ok=True)
				
				plasma_wallpapers_target = self.target / 'usr/share/plasma/wallpapers/fluxos'
				plasma_wallpapers_target.mkdir(parents=True, exist_ok=True)
				
				images_source = config_source / 'sddm_theme/images'
				if images_source.exists():
					for image in images_source.iterdir():
						if image.is_file() and image.name.lower().endswith(('.png', '.jpg', '.jpeg')):
							# Copy to multiple wallpaper locations
							for target_dir in [kde_wallpapers_target, plasma_wallpapers_target]:
								target_file = target_dir / image.name
								with image.open('rb') as src, open(target_file, 'wb') as dst:
									dst.write(src.read())
				
				# Install FluxOS logo to system locations
				logo_targets = [
					self.target / 'usr/share/icons/fluxos',
					self.target / 'usr/share/pixmaps/fluxos', 
					self.target / 'usr/share/logos/fluxos'
				]
				
				for logo_target in logo_targets:
					logo_target.mkdir(parents=True, exist_ok=True)
					if images_source.exists():
						for image in images_source.iterdir():
							if image.is_file() and 'logo' in image.name.lower():
								target_file = logo_target / image.name
								with image.open('rb') as src, open(target_file, 'wb') as dst:
									dst.write(src.read())
				
				info('Installed additional FluxOS wallpapers and logos')
				
			except Exception as e:
				warn(f'Failed to install KDE configs and additional assets: {e}')
			
			# Customize /etc/os-release for FluxOS branding
			try:
				os_release_file = self.target / 'etc/os-release'
				if os_release_file.exists():
					content = os_release_file.read_text()
					# Replace Arch Linux with FluxOS
					content = content.replace('NAME="Arch Linux"', 'NAME="FluxOS"')
					content = content.replace('PRETTY_NAME="Arch Linux"', 'PRETTY_NAME="FluxOS"')
					content = content.replace('ID=arch', 'ID=fluxos')
					content = content.replace('ID_LIKE=arch', 'ID_LIKE="arch fluxos"')
					if 'HOME_URL=' in content:
						content = re.sub(r'HOME_URL=.*', 'HOME_URL="https://fluxos.org/"', content)
					else:
						content += '\nHOME_URL="https://fluxos.org/"\n'
					os_release_file.write_text(content)
					info('Updated /etc/os-release for FluxOS branding')
			except:
				warn('Failed to update /etc/os-release')
			
		except Exception as e:
			error(f'Failed to install FluxOS configuration files: {e}')
