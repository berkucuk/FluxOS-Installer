...
class Tui:
	...
	def _set_up_colors(self) -> None:
		# FluxOS TUI color scheme: subtle turquoise (cyan) accents
		# Try to soften the cyan color into a turquoise tone when supported.
		# Note: init_color uses a 0-1000 range. This will gently shift cyan to a softer turquoise.
		try:
			if curses.can_change_color():
				# Slightly desaturated cyan → turquoise-like
				curses.init_color(curses.COLOR_CYAN, 330, 650, 700)
		except Exception:
			pass

		# NORMAL: white on black for body text
		curses.init_pair(STYLE.NORMAL.value, curses.COLOR_WHITE, curses.COLOR_BLACK)
		# CURSOR/FOCUS: turquoise highlight for focused items
		curses.init_pair(STYLE.CURSOR_STYLE.value, curses.COLOR_CYAN, curses.COLOR_BLACK)
		# MENU: inverted turquoise menu bar (text black on turquoise)
		curses.init_pair(STYLE.MENU_STYLE.value, curses.COLOR_BLACK, curses.COLOR_CYAN)
		# HELP: turquoise on black for help area
		curses.init_pair(STYLE.HELP.value, curses.COLOR_CYAN, curses.COLOR_BLACK)
		# ERROR: red on black unchanged
